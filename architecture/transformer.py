...
class MultiHeadCrossAttention(nn.Module):
    ...
    def get_attn(self, x):
        ...
        # Reshape the learnable query embeddings have dimensions 1, number of learnable query tokens, number of heads H (8), and dimnsional (113 or 512 depending on the task)
...
class MLP(nn.Module):
    ''' 
    MLP consists of two feed-forward layers which is used both after the heads are aggregated (as it was for the original implementation of IPS) 
    Importantly this MLP is also placed after each head individually to make an individual prediction for each head and compute the semantic loss
    '''
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Importantly for our implementation of Semantic loss we changed this script to return all of the attention heads in order to compute the semantic loss
